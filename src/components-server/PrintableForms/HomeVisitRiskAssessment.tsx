      ...
      sections: [
        {
          ...
          fields: [
            ...
            {
              label: "Any history of alcohol or drug use? (If yes, there can be no use of alcohol or use of drugs whilst the staff member is in home) ",
              key: "drugUseHistory",
            },
            {
              label: "Is there an advanced care directive? (If yes, please add this information to risk assessment and care plan) ",
              key: "careDirective",
            },
          ],
        },
        {
          ...
          fields: [
            ...
            {
              label: "Are there any weapons in the home? (If yes, please make sure they are stored appropriately during the visit.) ",
              key: "weaponsInHome",
            },
          ],
        },
      ...
      sections: [
        ...
        },
        {
          ...
          fields: [
            ...
            {
              label: "Are there any issues with mobile phone reception?",
              key: "mobileReception",
            },
          ],
        },
...
const Page1 = ({ homeVisitResponse, images, commonFields, settings }: any) => (
  <A4Page>
    ...
    <div className="px-6">
      <table
        className={`w-full border border-black ${A4_PDF_TYPOGRAPHY.tableCell} table-fixed`}
      >
        <tbody>
          <tr>
            ...
            <td className="border border-black p-2 w-1/3">
              <span className={A4_PDF_TYPOGRAPHY.label}>NDIS Number:</span>{" "}
              {commonFields?.ndis || homeVisitResponse.ndisNumber || ""}
            </td>
            <td className="border border-black p-2 w-1/3">
              <span className={A4_PDF_TYPOGRAPHY.label}>DOB:</span>{" "}
              {commonFields?.dob || homeVisitResponse.dob || ""}
            </td>
          </tr>
          <tr>
            <td className="border border-black p-2" colSpan={3}>
              <span className={A4_PDF_TYPOGRAPHY.label}>Address:</span>{" "}
              {commonFields?.street || homeVisitResponse.address || ""}
            </td>
          </tr>
          <tr>
            <td className="border border-black p-2" colSpan={3}>
              <span className={A4_PDF_TYPOGRAPHY.label}>
                Date of completion of risk assessment:
              </span>{" "}
    ...
    <div className="flex-1 px-6 py-4 flex flex-col">
      <table
        className={`w-full border-collapse border border-black ${A4_PDF_TYPOGRAPHY.tableCell} flex-1`}
      >
        <thead>
          <tr>
            <th
              className={`border border-black w-[40%] p-2 ${A4_PDF_TYPOGRAPHY.tableHeader}`}
            ></th>
            <th
              className={`border border-black w-[7%] text-center p-2 ${A4_PDF_TYPOGRAPHY.tableHeader}`}
            >
              YES
            </th>
            <th
            ...
            <th
              className={`border border-black w-[46%] p-2 ${A4_PDF_TYPOGRAPHY.tableHeader}`}
            >
              COMMENTS
            </th>
          </tr>
          ...
          {homeVisitSchema.pages[0].sections.map((section: any) => (
            <React.Fragment key={section.title}>
              <tr className="bg-gray-300">
                <td
                  className={`border border-black p-3 ${A4_PDF_TYPOGRAPHY.sectionHeader} text-left`}
                  colSpan={4}
                >
                  {section.title}
                </td>
              ...
              {section.fields.map((field: any) => (
                <tr key={field.key} className="align-top">
                  ...
                  <td className="border border-black text-center w-[7%]">
                    {homeVisitResponse[field.key]?.toLowerCase() === "yes" ? (
                      <svg
                  ...
                  <td className="border border-black text-center w-[7%]">
                    {homeVisitResponse[field.key]?.toLowerCase() === "no" ? (
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        width="12"
                  ...
                  <td
                    className={`border border-black p-3 w-[46%] ${A4_PDF_TYPOGRAPHY.tableCell}`}
                  >
              ...
              <React.Fragment key={sectionIndex}>
                {section.title && (
                  <tr className="bg-gray-300">
                    <td
                      colSpan={4}
                      className={`border border-black p-3 ${A4_PDF_TYPOGRAPHY.sectionHeader}`}
                    >
                      {section.title}
                    </td>
                  </tr>
                  ...
                  if (field.type === "checkboxGroup") {
                    return (
                      <tr key={field.key}>
                        ...
                        <td className="border border-black text-center align-top"></td>
                        <td
                          className={`border border-black p-3 align-top ${A4_PDF_TYPOGRAPHY.tableCell}`}
                        >
                          <div className="space-y-1">
                            {field.options.map((opt: any) => (
                              <label
                                key={opt}
                                className={`flex items-center ${A4_PDF_TYPOGRAPHY.small}`}
                              >
                                <input
                                  type="checkbox"
                                  checked={
                            ...
                            {comments && (
                              <div
                                className={`${A4_PDF_TYPOGRAPHY.small} mt-1 italic`}
                              >
                                {comments}
                              </div>
                            )}
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* <div className="px-6 py-4 bg-red-50 border-b border-red-200">
          <div className={`${A4_PDF_TYPOGRAPHY.sectionHeader} mb-2`}>
            ...
            <span className="text-red-600">
              {page.riskLevel.label.split(" ")[1]}
            </span>
          </div>
          <p
            className={`${A4_PDF_TYPOGRAPHY.body} text-gray-700 leading-relaxed`}
          >
        ...
        <div className="flex-1 px-6 pb-4 overflow-hidden flex flex-col">
          <div className="overflow-auto flex-1">
            <table
              className={`w-full border-collapse border border-black ${A4_PDF_TYPOGRAPHY.tableCell}`}
            >
              <thead>
                <tr className="bg-gray-400">
                  <th
                    className={`border border-black px-4 py-3 text-left ${A4_PDF_TYPOGRAPHY.tableHeader} text-black`}
                  >
                    Issue/Task
                  </th>
                  <th
    ...
    <div className="flex-1 px-12 pt-6 pb-8">
      <div className={`grid grid-cols-3 gap-12 ${A4_PDF_TYPOGRAPHY.body}`}>
        ...
        <div className="text-center">
          <div className={`${A4_PDF_TYPOGRAPHY.signatureLabel} mb-2`}>
            Name:
          </div>
          <div
            className={`border-b-2 border-black pb-1 min-h-[24px] ${A4_PDF_TYPOGRAPHY.signatureContent}`}
          >
        ...
        <div className="text-center">
          ...
          <div className="border-b-2 border-black pb-1 min-h-[50px] flex justify-center items-center">
            {homeVisitResponse?.signature ? (
              ...
              <span
                className={`text-gray-400 italic ${A4_PDF_TYPOGRAPHY.small}`}
              >
                No signature
              </span>
            )}
