  ...
  return (
    <>
      ...
      <div className=" py-4 sm:py-6 md:py-8">
        ...
        {/* Header */}
        <div className="bg-white rounded-lg shadow-sm border border-azure-200 p-4 sm:p-6 mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-azure-700">Employee Welcome Pack</h1>
              <p className="text-sm sm:text-base text-azure-400">{staff?.firstName} {staff?.surname}</p>
            </div>
            <button
              onClick={() => {
                const isSignatureLink = window.location.pathname.includes('/staff/signature/');
              ...
              className="px-4 py-2 text-sm text-azure-400 hover:text-azure-700 self-start sm:self-auto transition-colors"
            >
              ← Back to Forms
            </button>
          </div>
        ...
        {pdfPages.length > 0 && (
          <div className="space-y-4 sm:space-y-5 mb-6 sm:mb-8">
            {pdfPages.map((src, idx) => (
              <div
                key={idx}
                className="bg-white rounded-lg shadow-sm border border-azure-200 p-3 sm:p-4 flex justify-center"
              >
                <img
                  src={src}
                  alt={`PDF page ${idx + 1}`}
        ...
        <div className="flex justify-center mb-4 sm:mb-6">
          <div 
            className="bg-white shadow-sm border border-azure-200 mx-auto a4-ack-form"
            style={{
              width: '100%',
              maxWidth: '794px',
              minHeight: '1123px',
          ...
          >
            ...
            <div className="space-y-5" style={{ marginTop: '40px' }}>
              ...
              <div className="flex items-start gap-3 p-4 border border-azure-300 rounded-lg bg-azure-50 mb-6">
                <input
                  id="readAcknowledgement"
                  type="checkbox"
                  checked={formData.readAcknowledgement}
                  onChange={(e) => handleChange('readAcknowledgement', e.target.checked)}
                  className="mt-1 w-5 h-5 x-mark rounded border-azure-300 focus:ring-gold-500 focus:ring-2 cursor-pointer"
                />
              ...
              <div className="flex items-end" style={{ gap: '16px', marginBottom: '20px' }}>
                ...
                <div className="flex-1 border-b border-dotted border-azure-900" style={{ minHeight: '20px', paddingBottom: '4px', paddingLeft: '8px', backgroundColor: '#f9fafb' }}>
                  <input
                    type="text"
              ...
              <div className="flex items-end" style={{ gap: '16px' }}>
                ...
                <div className={`flex-1 border-b border-dotted ${!formData.date ? 'border-red-500' : 'border-azure-900'}`} style={{ minHeight: '20px', paddingBottom: '4px', paddingLeft: '8px' }}>
                  <input
                    type="date"
                    value={formData.date}
        ...
        {pdfPages.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-azure-200 p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <button
              onClick={() => handleSave(false)}
              disabled={saving}
              className="px-6 py-2 text-sm sm:text-base bg-azure-500 text-white rounded-lg hover:bg-azure-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
            ...
            <button
              ...
              disabled={saving}
              className="px-6 py-2 text-sm sm:text-base bg-azure-500 text-white rounded-lg hover:bg-azure-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
