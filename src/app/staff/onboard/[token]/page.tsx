        ...
        <div className="sticky top-0 z-20 bg-white py-3 border-b mb-4">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div className="w-1/2 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className={`h-full bg-rose-600 ${step===1 ? 'w-1/2' : 'w-full'}`} />
            </div>
          </div>
        </div>
          ...
          <>
            ...
            <div className="flex justify-end">
              <button className="px-6 py-3 bg-rose-600 text-white rounded-lg" disabled={!canContinue} onClick={async()=>{
                ...
                if (ok) setStep(1); else showToast({ type: 'error', title: 'Save failed', message: 'Please try again.' });
              }}>Next</button>
            </div>
          </>
        )}
        ...
        {step === 1 && (
          <>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button className="px-6 py-3 bg-rose-600 text-white rounded-lg" disabled={!canContinue} onClick={async()=>{
                if (!(detailsRef.current as any)?.validate?.()) {
                  showToast({ type: 'warning', title: 'Incomplete', message: 'Please fill all required fields correctly before continuing.' });
                  return;
                }
