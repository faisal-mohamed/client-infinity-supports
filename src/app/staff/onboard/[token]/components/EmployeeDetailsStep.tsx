      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        <div className="col-span-2 flex items-center gap-6">
          <div className="flex items-center gap-2 w-1/2">
            ...
            <select className="flex-1 border-b border-azure-400 px-1 py-1" value={data.gender || ''} onChange={(e)=>set('gender', e.target.value)} title="Gender">
        ...
        {/* Bank Details */}
        <div className="col-span-2 border border-azure-400 rounded-sm p-4 mt-2">
        ...
        <div className="col-span-2 mt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <textarea id="fld-restrictions" title="Any restrictions" placeholder="Any restrictions" className="w-full border border-azure-400 rounded px-2 py-2" value={data.workRestrictions || ''} onChange={(e)=>set('workRestrictions', e.target.value)} rows={3} />
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        <div className="col-span-2 mt-4">
          <div className="flex items-center gap-2 mb-2">
            <div className="font-semibold text-azure-600">Office Use Only</div>
            <span className="text-xs bg-gold-100 text-gold-700 px-2 py-1 rounded-full border border-gold-300">
              ⚠️ Admin Only - Do Not Edit
            </span>
          </div>
          <fieldset disabled className="border border-azure-400 p-4 mt-2 rounded-sm bg-azure-50 opacity-60 cursor-not-allowed">
            <legend className="text-xs text-azure-400 px-2">This section will be filled by office administrator</legend>
            <div className="text-xs font-semibold mb-3 text-azure-400">Employee:</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {/* Status styled as checkboxes to match PDF visuals (exclusive selection logic) */}
              <div>
                <div className="text-xs mb-1 text-azure-400">Status:</div>
                <div className="flex flex-col gap-2 pl-1">
                  <label className="inline-flex items-center gap-2 text-azure-400">
              ...
              <div className="space-y-2">
                <div className="flex items-center">
                  <label className="text-xs w-28 text-azure-400">Pay Rate</label>
                  <div className="flex-1 border-b border-azure-400 px-1 py-1 bg-azure-100 text-xs text-azure-600">
                    {data.payRate || ''}
                  </div>
                </div>
                <div className="flex items-center">
                  <label className="text-xs w-28 text-azure-400">SCHADS Level</label>
                  <div className="flex-1 border-b border-azure-400 px-1 py-1 bg-azure-100 text-xs text-azure-600">
            ...
            {data.adminSignature && (
              <div className="mt-4 pt-4 border-t border-azure-300">
                <div className="text-xs font-semibold mb-2 text-emerald-700">✅ Approved by Admin</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <div className="text-xs text-azure-400 mb-1">Admin Signature:</div>
                    <div className="border border-azure-300 rounded p-2 bg-white flex items-center justify-center h-20">
                      <img src={data.adminSignature} alt="Admin Signature" className="max-h-16 max-w-full" />
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-azure-400 mb-1">Approved Date:</div>
                    <div className="border-b border-azure-400 px-1 py-1 bg-white text-xs text-azure-600">
...
function Field({ label, value, onChange, type='text', className='', readOnly=false }: { label: string; value: any; onChange: (v:any)=>void; type?: string; className?: string; readOnly?: boolean }) {
  return (
    <div className={`mb-3 ${className}`}>
      ...
      <input id={`fld-${label}`} title={label} placeholder={label} className={`w-full border-b px-1 py-1 ${readOnly? 'bg-azure-50 border-azure-300 text-azure-600' : 'border-azure-400'}`} value={value || ''} onChange={(e)=>onChange(e.target.value)} type={type} readOnly={readOnly} />
    </div>
  );
}
      ...
      <div className="flex flex-wrap gap-1 items-center">
        {cells.map((ch, i) => (
          <input key={i} title={`${label} ${i+1}`} value={ch} maxLength={1} onChange={(e)=>{
            ...
            onChange(next);
          }} className="w-8 h-8 border border-azure-400 text-center placeholder-azure-300" placeholder="•" />
        ))}
...
function SelectField({ label, value, onChange }: { label: string; value: any; onChange: (v: boolean|null)=>void }) {
  return (
    <div>
      ...
      <select title={label} className="w-full border-b border-azure-400 px-1 py-1" value={value===true?'Yes':value===false?'No':''} onChange={(e)=>{
...
function NumberLineField({ label, value, onChange, maxLength=10 }: { label: string; value: string; onChange: (v: string)=>void; maxLength?: number }) {
  return (
    <div className="mb-3">
      ...
      <input id={`num-${label}`} title={label} placeholder={label} inputMode="numeric" pattern="[0-9]*" maxLength={maxLength} className="w-full border-b border-azure-400 px-1 py-1" value={value} onChange={(e)=>{
