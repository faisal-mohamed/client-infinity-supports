...
export const getStaffFormComponent = (formKey: string, mode: 'view' | 'edit' = 'view') => {
  // Resolve aliases from DynamoDB formType to registry key
  const aliases: Record<string, string> = {
    'employment_details': 'employee_details',
    'employment_welcome_ack': 'employee_welcome',
    'employment_welcome': 'employee_welcome',
  };
  const resolvedKey = aliases[formKey] || formKey;
  const item = staffFormRegistry[resolvedKey];
...
export const getStaffFormConfig = (formKey: string): StaffFormRegistryItem | undefined => {
  const aliases: Record<string, string> = {
    'employment_details': 'employee_details',
    'employment_welcome_ack': 'employee_welcome',
    'employment_welcome': 'employee_welcome',
  ...
  const resolvedKey = aliases[formKey] || formKey;
  return staffFormRegistry[resolvedKey];
};
