                ...
                <tr key={item.codeKey} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    <input
                      ...
                      className="w-full border border-azure-200 rounded px-2 py-1 text-sm"
                    />
                  </td>
                  <td className="px-4 py-2">
                    <input
      ...
      <div className="flex flex-col gap-1">
        ...
        {type === "textarea" ? (
          <textarea
            ...
            rows={4}
            className={`w-full rounded-lg border border-azure-100 bg-white px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent transition-all placeholder-azure-300 resize-y ${
              fieldErrors[name]
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span
            className={`text-sm leading-relaxed ${
              isCommon ? "text-azure-700" : "text-azure-600"
            }`}
          >
            {label}
            {required && <span className="text-red-500 ml-1">*</span>}
  ...
  const renderDropdown = (
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <label className="text-xs font-medium text-azure-600 mb-1 flex flex-wrap gap-1">
          <span
            dangerouslySetInnerHTML={{
              __html: label.replace(
                /\*\*(.*?)\*\*/g,
                '<strong class="text-azure-700 font-semibold">$1</strong>'
              ),
            }}
          />
