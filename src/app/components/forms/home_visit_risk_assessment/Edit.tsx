      ...
      <main className="w-full flex flex-col items-center justify-center flex-1">
            ...
            {FORM_SECTIONS[currentStep].image && (
              <div className="mt-4 w-full flex justify-center">
                <img
                  ...
                  className="w-full max-w-4xl h-auto rounded-xl border border-azure-100 shadow-md object-contain"
                ...
                <div className="space-y-6">
                  {Array.from({ length: riskEntryCount }, (_, i) => i + 1).map((num) => (
                    <div key={num} className="border border-azure-100 rounded-lg p-4 bg-azure-50">
                      <h3 className="text-lg font-semibold mb-4 text-azure-700">Risk Assessment Entry {num}</h3>
        ...
        <footer className="w-full max-w-2xl mx-auto bg-white/90 backdrop-blur-lg border-t border-azure-50 px-4 md:px-10 py-5 flex flex-col items-center gap-4 shadow-2xl rounded-b-3xl animate-fade-in mt-2">
          ...
          {currentStep === FORM_SECTIONS.length - 1 && (
            <button
              className="w-full mt-3 flex items-center justify-center gap-2 px-4 py-2 rounded-full font-semibold text-sm bg-gradient-to-r from-azure-600 to-emerald-400 text-white hover:from-azure-700 hover:to-emerald-500 shadow transition disabled:opacity-50"
              onClick={async (e) => {
                e.preventDefault();
                if (buttonsLocked) return;
