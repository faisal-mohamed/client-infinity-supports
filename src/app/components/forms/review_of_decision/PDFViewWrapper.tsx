...
const PDFViewWrapper: React.FC<PDFViewWrapperProps> = ({ formData, commonFieldsData }) => {
    ...
    const [instance] = usePDF({
        document: <ReviewOfDecisionPDF formData={formData} commonFieldsData={commonFieldsData} />
    });
            ...
            <Document
                ...
                loading={
                    <div className="flex flex-col items-center justify-center py-20 text-azure-400">
                        <div className="w-8 h-8 border-4 border-t-azure-600 border-azure-100 rounded-full animate-spin mb-4"></div>
                        <p>Rendering Pages...</p>
                    </div>
                }
            >
                {numPages && Array.from(new Array(numPages), (el, index) => (
                    <div key={`page_${index + 1}`} className="shadow-lg rounded-sm overflow-hidden bg-white">
                        <Page
                            pageNumber={index + 1}
                            width={800} // Standard width for desktop viewing
