...
export default function ClientsPageClient() {
    ...
    return (
      <Listbox value={value} onChange={onChange}>
        <div className="relative">
          <Listbox.Button aria-label={ariaLabel} className="w-full bg-white rounded-lg border border-gray-200 px-3 py-2 pr-8 text-sm text-gray-700 text-left focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-colors">
          ...
          <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
            <Listbox.Options anchor="bottom start" className="z-50 w-[var(--button-width)] overflow-auto rounded-lg border border-gray-200 bg-white py-1 text-sm shadow-elevated focus:outline-none [--anchor-gap:4px] [--anchor-max-height:240px]">
              {options.map((opt) => (
                <Listbox.Option key={opt.value + opt.label} value={opt.value} className={({ active, selected }) => `cursor-pointer select-none px-3 py-2 ${selected ? "bg-brand-50 text-brand-700 font-medium" : active ? "bg-gray-50" : "text-gray-700"}`}>
                  {opt.label}
                </Listbox.Option>
              ))}
            </Listbox.Options>
  ...
  const SortIcon = ({ field }: { field: string }) => {
    if (sortField !== field) return <FaSort className="w-3 h-3 text-gray-300" />;
    return sortDirection === "asc" ? <FaSortUp className="w-3 h-3 text-brand-600" /> : <FaSortDown className="w-3 h-3 text-brand-600" />;
  };
    ...
    <div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">Manage your clients and their information efficiently</p>
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
            <Link href="/admin/dashboard" className="flex items-center gap-2 text-sm text-gray-600 hover:text-brand-600 px-3 py-2.5 border border-gray-200 rounded-lg hover:bg-gray-50 justify-center transition-colors">
              <FaArrowLeft className="h-3.5 w-3.5" />
              Back to Dashboard
            </Link>
            <Link href="/admin/clients/create" className="flex items-center gap-2 text-sm text-white bg-brand-600 hover:bg-brand-700 px-4 py-2.5 rounded-lg font-medium justify-center transition-colors">
        ...
        <div className="p-4">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-3 py-2 border rounded-lg text-sm transition-colors ${showFilters ? "bg-brand-50 border-brand-200 text-brand-700" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                <FaFilter className="w-3 h-3" />
                Filters
                {Object.values(filters).some((v) => v !== "") && (
                  <span className="bg-brand-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{Object.values(filters).filter((v) => v !== "").length}</span>
                )}
              </button>
              <button type="button" onClick={exportToExcel} disabled={excelLoading} className="flex items-center gap-2 px-3 py-2 border border-gray-200 text-gray-600 rounded-lg text-sm hover:bg-gray-50 transition-colors">
                <FaDownload className="w-3 h-3" />
                {!excelLoading ? "Download Excel" : "Downloading..."}
              </button>
              {selectedClients.length > 0 && (
                <button type="button" onClick={handleDeleteSelected} disabled={isDeleting} className="flex items-center gap-2 px-3 py-2 border border-red-200 text-red-600 rounded-lg text-sm hover:bg-red-50 transition-colors">
          ...
          {showFilters && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={resetFilters} className="text-xs text-brand-600 hover:text-brand-700 font-medium">Reset all</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Has NDIS</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Has Disability</label>
      ...
      {error && (
        <div className="bg-red-50 border border-red-100 rounded-xl p-4 mb-4 flex items-center gap-3">
          ...
          <button onClick={() => loadClients()} className="text-xs font-medium text-red-600 hover:text-red-700 px-3 py-1 border border-red-200 rounded-lg hover:bg-red-100 transition-colors">Retry</button>
        </div>
      )}
          ...
          <>
                      ...
                      return (
                        <tr key={client.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-4 py-3">
                            <input type="checkbox" className="rounded border-gray-300 text-brand-600 focus:ring-brand-500" checked={selectedClients.includes(client.id)} onChange={() => handleSelectClient(client.id)} />
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2.5">
                              <div className="w-8 h-8 flex items-center justify-center bg-brand-50 text-brand-700 rounded-lg text-xs font-semibold">
                                {fullName.charAt(0).toUpperCase()}
                              </div>
                              <span className="font-medium text-gray-900">{fullName}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-gray-600">{client.phone || <span className="text-gray-300">—</span>}</td>
                          <td className="px-4 py-3">
                            {state ? (
                              <span className="inline-flex text-xs font-medium bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md">{stateMapping[state as keyof typeof stateMapping] || state}</span>
                            ) : <span className="text-gray-300">—</span>}
                          </td>
                          <td className="px-4 py-3 text-gray-500 text-xs">
                            <div>{new Date(client.createdAt).toLocaleDateString()}</div>
                            <div className="text-gray-400">{new Date(client.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
                          </td>
                          <td className="px-4 py-3 text-right">
                            <Menu as="div" className="relative inline-block text-left">
                              <MenuButton className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
                                <FaEllipsisV className="w-3.5 h-3.5" />
                              </MenuButton>
                              <MenuItems anchor="bottom end" className="w-36 bg-white border border-gray-200 rounded-lg shadow-elevated focus:outline-none z-50 py-1 [--anchor-gap:4px]">
                                <MenuItem>{({ active }) => (<Link href={`/admin/clients/${client.id}`} className={`flex items-center gap-2 px-3 py-2 text-sm ${active ? "bg-gray-50" : ""} text-gray-700`}><FaEye className="w-3.5 h-3.5" />View</Link>)}</MenuItem>
                                <MenuItem>{({ active }) => (<Link href={`/admin/clients/${client.id}/forms`} className={`flex items-center gap-2 px-3 py-2 text-sm ${active ? "bg-gray-50" : ""} text-gray-700`}><FaFileAlt className="w-3.5 h-3.5" />Forms</Link>)}</MenuItem>
            ...
            <div className="px-4 py-3 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3">
              <div className="flex items-center gap-3 text-sm text-gray-500">
                <span>Showing <span className="font-medium text-gray-700">{sortedClients.length}</span> of <span className="font-medium text-gray-700">{pagination.totalCount}</span></span>
                <select value={pagination.pageSize} onChange={handlePageSizeChange} className="border border-gray-200 rounded-md px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-brand-500">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handlePageChange(pagination.page - 1)} disabled={!pagination.hasPreviousPage} className="px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors">
                  Previous
                </button>
                <span className="text-xs text-gray-500">Page <span className="font-medium text-gray-700">{pagination.page}</span> of <span className="font-medium text-gray-700">{pagination.totalPages}</span></span>
                <button onClick={() => handlePageChange(pagination.page + 1)} disabled={!pagination.hasNextPage} className="px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors">
                  Next
                </button>
              </div>
            </div>
