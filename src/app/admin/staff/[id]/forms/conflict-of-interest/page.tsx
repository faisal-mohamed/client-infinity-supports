...
export default function AdminConflictOfInterestViewPage() {
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
    ...
    if (!adminFormData.reviewedBy?.trim() || 
        !adminFormData.reviewerTitle?.trim() || 
        !adminFormData.reviewerSignature ||
        !adminFormData.hrDecision) {
      showToast({
        type: 'error',
    ...
    <div className="">
      ...
      <div className="">
        ...
        {!hasEmployeeSignature && (
          <div className="bg-gold-50 border-2 border-gold-300 rounded-lg p-4 mb-4">
            <p className="text-sm text-gold-800 font-semibold">
        ...
        {hasEmployeeSignature && (
          <div className="bg-white rounded-lg shadow-soft p-3 sm:p-4 lg:p-6 mb-6">
            {rendering ? (
        ...
        {/* Admin Section - Show always, editable if not completed, simple message if completed */}
        {hasEmployeeSignature && (
          <div className="bg-white rounded-lg shadow-soft p-4 md:p-6">
            {hasAdminSignature ? (
              ...
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-lg p-4 flex items-center justify-between">
                ...
                <button
                  onClick={async () => {
                    if (confirm('Are you sure you want to clear the admin signature? This will allow you to edit the HR section.')) {
                      try {
                        setSaving(true);
                        const response = await fetch(`/api/staff/${staffId}/forms/conflict-of-interest/clear-admin-signature`, {
                          method: 'POST',
                        ...
                        if (response.ok) {
                          showToast({
                            ...
                            message: 'Admin signature has been cleared. You can now edit the HR section.',
                            duration: 3000,
                          });
              ...
              <>
            <div className="mb-6">
                  <h2 className="text-lg font-semibold text-azure-700 mb-2">HR/Management Review</h2>
              <p className="text-sm text-azure-400 mb-4">
            ...
            <form onSubmit={handleAdminSubmit} className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t">
                <button
                  type="submit"
                  disabled={saving}
                  className="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 w-full sm:w-auto"
                >
        ...
        {hasEmployeeSignature && (
          <div className="flex justify-center pt-4 sm:pt-6 mt-4 sm:mt-6 border-t">
            <button
              ...
              className="px-6 py-2 bg-azure-700 text-white rounded-lg hover:bg-azure-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {downloading ? (
                <>
