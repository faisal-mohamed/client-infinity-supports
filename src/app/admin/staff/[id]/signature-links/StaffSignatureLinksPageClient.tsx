...
export default function StaffSignatureLinksPageClient() {
  ...
  const copyLinkToClipboard = async (token: string) => {
    const link = `${window.location.origin}/staff/signature/${token}/forms`;
    try {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex items-center justify-center">
        <div className="text-center">
          <FaSpinner className="h-12 w-12 text-rose-500 animate-spin mx-auto mb-4" />
          <p className="text-slate-600 text-lg">Loading signature links...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6 border-2 border-blue-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-blue-600 text-sm font-semibold mb-1">Total Links</p>
            <p className="text-3xl font-bold text-blue-900">{signatureBatches.length}</p>
          </div>
          <div className="p-4 bg-blue-500 rounded-xl">
            <FaLink className="h-8 w-8 text-white" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-6 border-2 border-amber-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-amber-600 text-sm font-semibold mb-1">Expired Links</p>
        ...
        <div className="space-y-6">
          {signatureBatches.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-12 text-center">
              <FaInfoCircle className="h-16 w-16 text-slate-300 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-slate-700 mb-2">No Signature Links</h3>
              <p className="text-slate-500">
                No signature links have been generated for this staff member yet.
              </p>
              <Link
                href={`/admin/staff/${staffId}/forms`}
                className="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-rose-600 text-white rounded-xl font-semibold hover:bg-rose-700 transition-all duration-200"
              >
                  ...
                  <div className="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b border-slate-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className={`px-4 py-2 rounded-xl border-2 ${statusInfo.color} flex items-center gap-2`}>
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-semibold text-slate-700">Signature Link</span>
                      </div>
                      <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 font-mono text-sm text-slate-600 break-all">
                        {`${typeof window !== 'undefined' ? window.location.origin : ''}/staff/signature/${batch.batchToken}/forms`}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                      <div>
                        ...
                        <p className="text-sm text-slate-600">{formatDate(batch.createdAt)}</p>
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                        ...
                        {isEditing ? (
                          ...
                          <div className="flex items-center gap-2">
                            <p className="text-sm text-slate-600">{formatDate(batch.expiresAt)}</p>
                            <button
                              onClick={() => startEditingExpiry(batch.id, batch.expiresAt)}
                              className="p-1 hover:bg-slate-100 rounded transition-colors"
                              title="Edit Expiry"
                            >
                              <FaEdit className="h-3 w-3 text-slate-400" />
                            </button>
                          </div>
                        )}
                        <p className="text-xs text-slate-500 mt-1">{getTimeRemaining(batch.expiresAt)}</p>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        <FaFileAlt className="h-4 w-4 text-slate-400" />
                        <span className="text-sm font-semibold text-slate-700">Forms ({totalForms})</span>
                      </div>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              {sf.formSubmission.staffSignature ? (
                                ...
                                <FaClock className="h-5 w-5 text-slate-400" />
                              )}
