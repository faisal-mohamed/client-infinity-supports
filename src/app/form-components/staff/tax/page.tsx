    ...
    <div className="absolute flex" style={{ top, left, gap: totalWidth ? 0 : gap }}>
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          className={`char-input border border-azure-200 text-center text-sm ${readOnly ? 'bg-gray-100 cursor-not-allowed' : ''}`}
      ...
      >
        ...
        <input
          ...
          maxLength={1}
            value={day[i] || ""}
            readOnly={true}
            className={`border border-azure-200 text-center text-sm ${readOnly ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'} ${required && !value ? 'border-red-500' : ''}`}
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl w-[400px] max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full px-4 py-2 border-2 border-azure-100 rounded-lg focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 text-base"
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 bg-azure-600 text-white rounded-lg hover:bg-azure-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
              >
                Save
              </button>
            </div>
          ...
          {Array.from({ length: cols }).map((_, colIdx) => {
            ...
            return (
              <input
                ...
                className={`border border-azure-200 text-center text-sm ${readOnly ? 'bg-gray-100 cursor-not-allowed' : ''}`}
        ...
        const boxes = Array.from({ length: group.length }).map((_, i) => {
          ...
          return (
            <input
              ...
              onChange={!readOnly ? (e) => handleChange(e.target.value, idx) : undefined}
              onKeyDown={!readOnly ? (e) => handleKeyDown(e, idx) : undefined}
              className={`border border-azure-200 text-center text-sm absolute ${readOnly ? 'bg-gray-100 cursor-not-allowed' : ''}`}
              style={{
                top: group.top,
      ...
      />
       ...
       {showButtons && (
         <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-4">
           <button
             onClick={handleSave}
             className="px-4 py-2 bg-azure-700 text-white rounded shadow hover:bg-azure-800"
           >
             Save
           </button>
           <button
