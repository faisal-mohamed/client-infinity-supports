...
function Field({ label, value }: { label: string; value?: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="border border-azure-200 h-auto min-h-[2rem] rounded-sm px-2 py-1 flex items-center text-gray-900 bg-white">
        {value || ''}
      </div>
    </div>
  );
...
function SignatureField({ label, image }: { label: string; image?: string | null }) {
  return (
    <div>
      ...
      <div className="border border-azure-200 h-16 flex items-center justify-center bg-white rounded-sm">
        {image ? (
