  ...
  const renderField = (label: string, field: keyof SupportWorkerFormData, type: string = 'text', required: boolean = false) => (
    <div className="space-y-1">
      ...
      <input
        ...
        className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-gold-500/30 ${
          errors[field] ? 'border-red-500' : 'border-azure-100'
        }`}
      />
  ...
  const renderCheckbox = (label: string, field: keyof SupportWorkerFormData, required: boolean = false) => (
    <div className="flex items-center space-x-2">
      <input
        ...
        disabled={!isFormEditable}
        className="w-4 h-4 text-azure-700 border-azure-100 rounded focus:ring-gold-500/30"
      />
    ...
    <div className="bg-gray-100 py-8">
      <FormPage title="Support Worker Application Form" meta={{ website: 'infinitysupportswa.org', version: 'SW001', reviewDate: '2025-03-01' }}>
        <div className="space-y-6">
          {/* Personal Information */}
          <div className="border border-azure-100 rounded-lg p-6 bg-white">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-gold-500/30 ${
                    errors.signature ? 'border-red-500' : 'border-azure-100'
                  } ${!isFormEditable ? 'bg-gray-100' : ''}`}
                />
          ...
          {!isFormEditable && (
            <div className="flex justify-end space-x-4 pt-6">
              ...
              <button
                ...
                className="px-6 py-2 bg-azure-700 text-white rounded-md hover:bg-azure-800 focus:outline-none focus:ring-2 focus:ring-gold-500/30 disabled:opacity-50"
              >
